...
- **credential-process**: CLI 工具，处理 OIDC 认证 -> AWS STS 临时凭证发放，同时执行配额检查
- **Landing page**: ALB + Lambda 门户，用户登录和状态查看
- **Admin panel**: 同一 ALB 下的独立 Lambda，管理配额策略、定价、管理员
- **OTLP pipeline**: otel-helper -> credential-process (JWT) -> OTEL collector -> CloudWatch -> MetricsAggregator Lambda -> DynamoDB
...
| 现象 | 原因 / 解决 |
|------|------------|
| Admin panel 显示 0 用户 | 检查 otel-helper 的 `AWS_PROFILE` 是否正确；检查 OTLP endpoint 端口是否为 80 |
| Aggregator 漏数据 | `AGGREGATION_WINDOW` 过小（默认 15 分钟已考虑 CW Insights 索引延迟） |
| 用户费用显示 $0 | 检查 BedrockPricing 表中的 model ID 是否正确 |
...
- **credential-process**: CLI tool that handles OIDC auth -> AWS STS credential issuance, with quota checks
- **Landing page**: ALB + Lambda portal for user login and status
- **Admin panel**: Separate Lambda on the same ALB for managing quotas, policies, pricing, and admins
- **OTLP pipeline**: otel-helper -> credential-process (JWT) -> OTEL collector -> CloudWatch -> MetricsAggregator Lambda -> DynamoDB
...
| Symptom | Cause / Fix |
|---------|-------------|
| Admin panel shows 0 users | Check otel-helper has correct `AWS_PROFILE`; check OTLP endpoint port is 80 |
| Aggregator misses data | `AGGREGATION_WINDOW` too small (default 15 min accounts for CW Insights indexing lag) |
| User cost shows $0 | Check BedrockPricing table has correct model IDs |
