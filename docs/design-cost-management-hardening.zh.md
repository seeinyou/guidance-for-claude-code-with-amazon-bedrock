...
| refresh_token | 1440 分钟（24 小时） | 无需浏览器即可换取新 id_token |
...
```
09:00  浏览器登录（第 1 次）🌐
09:00  → id_token（60 分钟）+ refresh_token（24 小时）→ TVM Lambda → Bedrock 凭证（1 小时）
10:00  Bedrock 凭证过期 → refresh_token → 新 id_token → TVM Lambda → 新 Bedrock 凭证（静默）
11:00  同上（静默）
...
09:00(+1d)  refresh_token 过期 → 浏览器登录（第 2 次）🌐
...
```
...
Cognito 配置：id_token=60 分钟，refresh_token=24 小时
...
11:00  静默 ✓
  ...
18:00  静默 ✓
09:00(+1d)  refresh_token 过期 → 浏览器登录 🌐
  ...
...
| 参数 | 当前值 | 配置位置 | 影响范围 | 说明 |
|------|--------|----------|----------|------|
| **Cognito refresh_token 有效期** | 24 小时（1440 分钟） | Cognito User Pool App Client | 用户多久需要重新浏览器登录 | `aws cognito-idp update-user-pool-client --refresh-token-validity 1440` |
...
| 凭证刷新机制 | Cognito refresh_token（24 小时） | 避免每小时弹出浏览器；兼容 OIDC 联合认证 |
...
| Cognito refresh_token 有效期 | 24 小时（1440 分钟） | 覆盖完整工作日，只需一次浏览器登录；从默认 10 小时改为 24 小时 |
...
**Cognito 配置：**
- [x] Cognito User Pool App Client — `RefreshTokenValidity` 改为 1440 分钟（24 小时）
  ...
  - **验证命令：**
    ...
    ```
- [ ] 验证：`describe-user-pool-client` 确认 RefreshTokenValidity=1440
...
- [ ] 等 refresh_token 过期（24 小时）→ 确认弹出浏览器重新登录
